...
class Solver:
    ...
    def single_solution(self, pos, piece_index):
        ...
        self.indexes = list(range(len(self.board.pieces)))
        self.current_cost = 0
    ...
    def build_around(self, pos):
        ...
        :return: 
        '''
        piece_index = self.board.board[pos].get_index()
        cur_piece = self.board.pieces[piece_index]
        ...
        if not len(self.indexes) == 0:
            ...
            if next_pos is not None:
                self.build_around(next_pos, self.board.board[next_pos])
    ...
    def assign_pieces(self, cur_pos, assign):
        for (index, direction) in assign:
            self.board.add_piece_in_direction(cur_pos, self.board.pieces[index], direction)
